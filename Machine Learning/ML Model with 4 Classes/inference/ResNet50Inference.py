...
def infer_folder(model, folder: str, device: torch.device, output_widget):
    ...
    counts = Counter()
    total = len(files)
    output_widget.insert(tk.END, f"Inferencing {total} images in '{folder}'\n\n")
    ...
    for fname in files:
        ...
        counts[cls] += 1
        pct = conf.item()*100
        output_widget.insert(tk.END, f"{fname:30s} → {cls:10s} ({pct:5.2f}%)\n")
...
class InferenceApp:
    def __init__(self, root):
        self.root = root
        root.title("Emotion Classifier Inference")

        tk.Label(root, text="Model checkpoint:").grid(row=0, column=0, sticky="e")
        self.ckpt_path_var = tk.StringVar()
        tk.Entry(root, textvariable=self.ckpt_path_var, width=50).grid(row=0, column=1)
        tk.Button(root, text="Browse…", command=self.browse_ckpt).grid(row=0, column=2)

        tk.Label(root, text="Images folder:").grid(row=1, column=0, sticky="e")
        self.folder_var = tk.StringVar()
        tk.Entry(root, textvariable=self.folder_var, width=50).grid(row=1, column=1)
        tk.Button(root, text="Browse…", command=self.browse_folder).grid(row=1, column=2)

        tk.Button(root, text="Run Inference", command=self.run_inference).grid(row=2, column=1, pady=5)
